    ...
    def process_item(self, item, spider):
        ...
        if self.is_new:
            ...
            try:
                ...
                #print(f"MySQL Error: {err}")
                query = "UPDATE count649 SET `{}`=`{}`+1, `{}`=`{}`+1, `{}`=`{}`+1, `{}`=`{}`+1, `{}`=`{}`+1, `{}`=`{}`+1 WHERE year={} and type='M'".format(
                    num[0],num[0],num[1],num[1],num[2],num[2],num[3],num[3],num[4],num[4],num[5],num[5],item['year'])
                #print(query)
                self.cursor.execute(query)
                self.conn.commit()
                #print(f"MySQL Error: {err}")
            ...
            # count649 테이블 Bonus 에 저장
            bonus = item['bonus']
            try:            
                query = "INSERT INTO count649 (year,type,`{}`) VALUES ({},'{}',{})".format(bonus,item['year'],'B',1)
                #print(query)
                self.cursor.execute(query)
                self.conn.commit()
            except mysql.connector.Error as err:
                ...
                #print(f"MySQL Error: {err}")
                query = "UPDATE count649 SET `{}`=`{}`+1 WHERE year={} and type='B'".format(bonus, bonus, item['year'])
                #print(query)
                self.cursor.execute(query)
                self.conn.commit()
                #print(f"MySQL Error: {err}")
